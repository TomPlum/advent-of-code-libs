...
| <pre lang="kotlin"><T> List<T>.cartesianProductCubic(second: List<T>, third: List<T>): List<Triple<T, T, T>></pre>                       | For three sets A, B and C, the Cartesian product of A, B and C is denoted by A×B×C and defined as ` A×B×C = { (p, q, r) \| pϵA and qϵB and rϵC }`. Put simply, the Cartesian Product is the multiplication of three sets to form the set of all ordered pairs. Returns the cartesian product of itself and the given sets, meaning both A, B and C are `this`, `second` and `third` respectively. |
...
| <pre lang="kotlin"><T> Collection<T>.split(predicate: (element: T) -> Boolean): Collection<Collection<T>></pre>                          | Splits a collection based on a given predicate.                                                                                                                                                                                                                                                                                                                                                   |
| <pre lang="kotlin"><L, R> Collection<String>.toVerticalLists(parse: (String) -> Pair<L, R>?): Pair<MutableList<L>, MutableList<R>></pre> | Parses a collection of Strings (Usually puzzle input lines) vertically to produce two lists.                                                                                                                                                                                                                                                                                                      |
